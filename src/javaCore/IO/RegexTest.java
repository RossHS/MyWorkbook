 ...
 * и сопоставляемые с ним символьные строки, а для выхода из
 * программы - нажать клавишу пробела. Если шаблон содержит
            ...
            if (matcher.matches()) {
                ...
                if (g > 0) {
                    for (int i = 0; i < input.length(); i++) {
                        //вывести любые пустые группы
                        for (int j = 1; j <= g; j++)
                            if (i == matcher.start(j) && i == matcher.end(j))
                                System.out.print("()");
                        //вывести знак ( в начале непустых групп
                        for (int j = 0; j <= g; j++)
                            if (i == matcher.start(j) && i != matcher.end(j))
                                System.out.print('(');
